    ...
    @if ($errors->any())
        ...
        <div class="container">
            <div class="p-4">
                ...
                <form class="gap-3" action="{{ route('products.update', $product) }}" method="POST"
                    ...
                    <div class="p-2">
                        ...
                        <select name="status" class="form-control select w-full" required>
                            <option value="draft"
                                    @if($product->status == 'draft') selected="selected" @endif>Draft
                            </option>
                            <option value="available"
                                    @if($product->status == 'available') selected="selected" @endif>Available
                            </option>
                            <option value="soon"
                                    @if($product->status == 'soon') selected="selected" @endif>Soon
                            </option>
                            <option value="running_out"
                                    @if($product->status == 'running_out') selected="selected" @endif>Running Out
                            </option>
                        </select>
                    </div>
                        ...
                        <select name="category_id" id="category_id" class="form-select">
                            @foreach ($categories as $category)
                                <option value="{{ $category->id }}"
                                        @if($product->category_id == $category->id) selected="selected" @endif>{{ $category->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="p-2">
                        ...
                        <textarea name="desc" id="desc" class="ckeditor form-control" placeholder="Text"
                                  rows="8" required>{{ old('desc', $product->desc) }}
                        </textarea>
                    </div>
